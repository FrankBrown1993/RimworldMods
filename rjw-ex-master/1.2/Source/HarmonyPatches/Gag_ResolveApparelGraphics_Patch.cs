	...
	public static class ResolveApparelGraphics_Patch
	{
		...
		public static bool RJWExTryGetGraphicApparel(Apparel apparel, BodyTypeDef bodyType, out ApparelGraphicRecord rec)
		{
			...
			else
			{
				...
				string headGraphicPath = pawn.story.HeadGraphicPath; //path containing Normal/Pointy/Wide
				string gender = pawn.gender.ToString(); //Female/Male/None
				string crownType = pawn.story.crownType.ToString(); //Narrow/Average  TODO check

				//checks====
				if (headGraphicPath.NullOrEmpty() || apparel.def.apparel.wornGraphicPath.NullOrEmpty() || pawn.story.crownType == CrownType.Undefined)
