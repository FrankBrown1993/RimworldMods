...
namespace rjw
{
	...
	public class SexGizmo : Gizmo
	{
		...
		public override GizmoResult GizmoOnGUI(Vector2 topLeft, float maxWidth)
		{
			...
			rect5.width = 100f;
			rect5.height = 22f;
			float duration = ((JobDriver_Sex)pawn.jobs?.curDriver).duration;
			//float ticks_left = ((JobDriver_Sex)pawn.jobs?.curDriver).ticks_left;
			float sex_ticks = ((JobDriver_Sex)pawn.jobs?.curDriver).sex_ticks;
			float orgasm = 0;
			...
			if (!MP.IsInMultiplayer)   //TODO: someday write mp synchronizers?
				if (pawn.IsColonistPlayerControlled && pawn.CanChangeDesignationColonist())
				{
					...
					if (pawn.jobs?.curDriver is JobDriver_SexBaseInitiator)
					{
						...
						if (Widgets.ButtonImage(rect9, !((JobDriver_Sex)pawn.jobs?.curDriver).neverendingsex ? this.LimitedTex : this.UnlimitedTex, true))
						{
							if (pawn.jobs?.curDriver is JobDriver_SexBaseInitiator)
							{
								((JobDriver_Sex)pawn.jobs?.curDriver).neverendingsex = !((JobDriver_Sex)pawn.jobs?.curDriver).neverendingsex;
								if (((JobDriver_Sex)(pawn.jobs?.curDriver)).neverendingsex)
								{
					...
					{
						if (((JobDriver_Sex)pawn.jobs?.curDriver).isRape)   //TODO: beating interrupts loving, fix someday?
						{
							float num = 32f;
							float num2 = 34f;
							float num3 = rect2.height / 2f - num + num2;
							...
							{
								((JobDriver_Sex)pawn.jobs?.curDriver).beatonce = true;
