...
namespace rjw
{
	...
	static class RMB_Menu
	{
		...
		public static List<FloatMenuOption> GenerateSexOptions(Pawn pawn, LocalTargetInfo target)
		{
			...
			// TODO: figure out how to write mp sync or w/e causing error, maybe in 2021?
				...
				if (target.Pawn != null)
					...
					if (xxx.can_rape(pawn, true))
					{
						...
						(xxx.is_human(target.Pawn) || (xxx.is_animal(target.Pawn) && RJWSettings.bestiality_enabled)))
						{
							...
							else if (target.Pawn.IsDesignatedComfort() && RJWSettings.colonist_CP_rape)
							{
		...
		public static List<FloatMenuOption> GenerateSexRoleOptions(Pawn pawn, LocalTargetInfo target, bool rape, JobDef job)
		{
			List<FloatMenuOption> opts = new List<FloatMenuOption>();
			FloatMenuOption option = null;
			...
			for (int i = 0; i < SexUtility.rmb_dictionarykeys.GetLength(0); i++)
			{
				...
				if (dictionaryKey != null)
				{
					...
					var prefix = "Do ";
					prefix = dictionaryKey.GetModExtension<InteractionExtension>().RMBLabelM.CapitalizeFirst();
					var postfix = "";
					//if (rape)
					//	prefix = " - Rape";
					var sextype = i;
					...
					if (sextype % 2 != 0)   //receive action/get penetrated
						if (!rape)
						{
							giving = partner;
							receiving = pawn;
							prefix = "Ask for ";
							if (arraytype == 2)		//bestiality
								prefix = "Provide for ";

							prefix = dictionaryKey.GetModExtension<InteractionExtension>().RMBLabelF.CapitalizeFirst();
							postfix = "";
						}
						else if (rape)
						{
					...
					option = FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption(prefix + postfix, delegate ()
					//option = FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("Commit " + dictionaryKey.LabelCap + t1, delegate ()
					{
						HaveSex(pawn, rape, job, target, dictionaryKey, giving, receiving);
						//HaveSex2(pawn, rape, job, partner, dictionaryKey, giving, receiving);
		...
		[SyncMethod]
		static void HaveSex(Pawn pawn, bool rape, JobDef jobDef, LocalTargetInfo target, InteractionDef dictionaryKey, Pawn giving, Pawn receiving)
		{
			Pawn partner;
			if (target.Thing is Corpse)
				partner = (target.Thing as Corpse).InnerPawn;
