...
namespace rjw
{
	public static class AfterSexUtility
	{
		...
		public static void think_about_sex(Pawn pawn, Pawn partner, bool isReceiving, SexProps props, bool whoring = false)
		{
			...
			if (thoughtgain != null)
				pawn.needs.mood.thoughts.memories.TryGainMemory(thoughtgain, partner);
		}
			...
			if (stage >= 3)
			{
				...
				if (RJWSettings.AddTrait_Masocist && !xxx.is_masochist(pawn))
				{
					...
					if (Rand.Chance(chance))
					{
						...
						{
							pawn.story.traits.GainTrait(new Trait(xxx.masochist));
		...
		public static void processBrokenPawn(Pawn pawn)
		{
			...
			if (xxx.is_human(pawn) && !pawn.Dead && pawn.records != null)
			{
				if (xxx.has_traits(pawn))
				{
					...
					{
						float num = xxx.feelingBroken.initialSeverity;
						...
						pawn.health.hediffSet.GetFirstHediffOfDef(xxx.feelingBroken).Severity += num;
					}
					BadlyBroken(pawn);
				}
			}
